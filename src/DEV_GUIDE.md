...
**This is critical for a scalable application.** MongoDB does not automatically index fields like Firestore does. To ensure fast queries, you must manually create indexes in your MongoDB Atlas cluster for any fields you search or sort by frequently.

**Recommended Indexes to Create:**

*   **`users` collection:**
    *   `email`: (unique) - Essential for fast login lookups.
*   **`books` collection:**
    *   `type`
    *   `sellerId`
    *   `genre`
